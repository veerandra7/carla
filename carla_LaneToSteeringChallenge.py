...
def main():
    ...
    #ego_cam.listen(lambda img: img.save_to_disk('~/tutorial/output/%.6d.jpg' % img.frame))
    ego_cam.listen(lambda image : process_img(image))
    #cap = ego_cam.listen(lambda image : process_img(image))


    
    

    #cap = cv2.imread(input_tensor)
    #cap = cv2.VideoCapture(input_tensor)
    #img1 = np.asarray(ego_cam.listen(lambda image : process_img(image)))
    img1 = np.asarray(ego_cam.listen(lambda image : process_img(image.raw_data)))
    input_tensor = tf.convert_to_tensor(img1)
